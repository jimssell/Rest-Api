...
class Mortgage_form_Table extends WP_List_Table {
	
	function __construct(){
		global $status, $page;
			parent::__construct( array(
				'singular'  => "Control Panel",
				'plural'    => "Control Panel",
				'ajax'      => false
...
/*	function get_columns(){
	...
	function get_columns(){
		...
		$columns = array(
			...
			'name' => 'Server name',
			'email' => 'IP address',
			'phone'  =>'Reseller (current/max)',
			'credit' => 'C/R',
			'incomplete_form' => 'Customers (current/max)',
			'phone' => 'Status',			
	...
	function column_id($item){
		...
		$actions = array(
			...
			'view'    => sprintf('<a href="javascript:void(0);" onclick="view_this_item_mortgage(%s)">View</a>',$item->id, 'view',$item->id),
	...
	function prepare_items() {
		...
		/* -- Preparing your query -- */
			$query = "SELECT * FROM ".$prefix."mortgage_form_record where 1=1 ".$where;
		...
		switch( $column_name ) { 
			...
			case 'date_added':
				/*return date('F j, Y, g:i a');*/
                                ...
                                return get_date_from_gmt( $item->date_added, "F j, Y, g:i a" );
                            	//return _e($item->$column_name,'homemortgagebank');	
			default:
...
$wp_list_table = new Mortgage_form_Table();
$wp_list_table->prepare_items();
?>
<!-- Nav bar -->
<div class="wrap">
<h2 id="setting_nav" class="nav-tab-wrapper"> 
<span class="nav-tab nav-tab-active">Control Panel</span> 
<a class="nav-tab" href="admin.php?page=reseller_server_+allocation_request">Reseller Server Allocation Request</a> 
<a class="nav-tab" href="admin.php?page=server_overview">Server OverView</a> 
<a class="nav-tab" href="admin.php?page=reseller_allocation_form">Reseller Allocation</a></h2>
<h1></h1>
